...
inputDialogues4 = LOAD 'hdfs:///user/shwetha/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, dialogue:chararray);
inputDialogues5 = LOAD 'hdfs:///user/shwetha/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, dialogue:chararray);
inputDialogues6 = LOAD 'hdfs:///user/shwetha/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, dialogue:chararray);
...
rmf hdfs:///user/shwetha/outputs;

--Store the count in a folder
STORE namesOrder INTO 'hdfs:///user/shwetha/outputs' USING PigStorage('\t');